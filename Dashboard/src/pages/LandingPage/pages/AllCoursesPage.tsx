...
function CourseCardSkeleton() {
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden animate-pulse flex flex-col transform transition-all duration-300 hover:scale-[1.02]">
    ...
    const fetchEnrolledCourses = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setEnrolledCourses(data.map((entry: any) => entry.course._id));
        }
      } catch (error) {
      ...
      <aside className="hidden md:block w-72 shadow-xl p-6 sticky mt-4 ml-8 border border-gray-200 rounded-lg">
        ...
        <div className="mb-8">
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto">
            ...
            {categories.map(category => (
              <label key={category} className="flex items-center p-2 rounded-md hover:bg-gray-50 transition-colors">
                <input
                  type="radio"
                  name="category"
                  value={category}
        ...
        <div className="mb-8">
          ...
          <div className="space-y-2">
            ...
            {levels.map(level => (
              <label key={level} className="flex items-center p-2 rounded-md hover:bg-gray-50 transition-colors">
                <input
                  type="radio"
                  name="level"
                  value={level}
        ...
        <button
          className="flex items-center justify-center w-full py-3 bg-gray-100 text-gray-700 font-semibold rounded-md hover:bg-gray-200 transition-colors"
          ...
          {loading ? (
            ...
            <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {filteredCourses.map(course => {
                const isEnrolled = user?.id && enrolledCourses.includes(course._id);
                return (
                  <div
                    key={course._id}
                    className="bg-white rounded-lg border border-gray-200 overflow-hidden hover:border-gray-300 hover:shadow-md transition-all duration-200 group cursor-pointer flex flex-col"
                  ...
                  >
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      <div className="justify-between flex mb-2">
                        ...
                        <span className={`px-3 py-1 rounded-md text-xs font-medium capitalize ${getLevelColor(course.level)}`}>
                          {course.level}
                        </span>
                      </div>
                     ...
                     <div className="flex items-center mt-auto">
                      <button
                        className="flex items-center bg-gray-900 text-white py-2 px-2 rounded-md hover:bg-gray-800 transition-colors duration-200 font-medium text-sm"
                        onClick={e => {
                          e.stopPropagation();
                          navigate(`/courses/${course._id}`);
                        }}>
