...
const CoursesSection = () => {
  ...
  const [courses, setCourses] = useState([]); // State to store API data
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        // Filter courses where topRated is true
        const topRatedCourses = data.filter((course) => course.topRated === true);
  ...
  const mapCourseData = (course) => ({
    ...course,
    subtitle: 'Complete Guide to Mastery', // Adjust based on your needs
    totalHours: `${course.duration} hours`,
    students: '12,547', // Placeholder, update if API provides this
    instructorName: course.instructor?.name,
    instructorTitle: 'AI Expert', // Placeholder, update if API provides this
    popular: course.inDemand || false,
    originalPrice: `₹${course.price + 1000}`, // Example logic for original price
    thumbnail: {
      ...course.thumbnail,
      url: course.thumbnail.url || 'https://via.placeholder.com/480x360', // Fallback image
    },
  });
                    ...
                    <div className="flex items-center space-x-3 mb-4 p-3 bg-blue-50 rounded-lg">
                      <div className="w-8 h-8 bg-gradient-to-r from-blue-600 to-purple-600 rounded-full flex items-center justify-center">
                        <span className="text-xs font-bold text-white">
                          {mappedCourse.instructorName?.split(' ').map((n) => n[0]).join('')}
                        </span>
                      </div>
                      <div>
  ...
  onClick={() => navigate(`/courses/${mappedCourse._id}`)} // ✅ navigate by ID
>
              ...
              >
                {courses.map((course, index) => {
                  ...
                  return (
                    ...
                    >
                      <div className="relative overflow-hidden transition-all duration-500 shadow-xl bg-white backdrop-blur-sm max-w-72 rounded-2xl border border-gray-200">
