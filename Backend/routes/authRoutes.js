...
import { register, login, logout, changePassword, getUserDetails, getUserById , getAllAdmins, manageInstructorUpgrade, getPendingUpgradeRequests, requestInstructorUpgrade, updateUserInfo, deleteSubadmin, syncAuth0User, assignRole } from '../controllers/authController.js';
...
router.post("/request-instructor-upgrade", requestInstructorUpgrade); // No auth required
router.get("/pending-upgrade-requests", verifyAuth0Token, getPendingUpgradeRequests);
router.post("/manage-instructor-upgrade", verifyAuth0Token, manageInstructorUpgrade);
